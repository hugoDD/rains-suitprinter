...
public class HtmlUtil {
    ...
    public static String buildHtml(String json, Map<String, DataVO> dataMap) throws Exception {
        ...
        DataVO value;
        for (Layer vo : design.getLayers()) {
            value = dataMap.get(vo.getText());
            value = value == null ? new DataVO(DataItemUtil.build(vo.getText())) : value;
     ...
     * @return
     */
    private static String buildDiv(DesignVO design, Layer vo, DataVO dataVO) {
        // 空检查
        ...
        if (items != null) {
            ...
            for (int i = 0; i < items.length; i++) {
              //  occupy.setX(vo.getY() + i * (vo() + occupy.getHeight()));
